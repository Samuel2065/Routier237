...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-header">
            <h4><i class="bi bi-person-workspace"></i> Routier+237</h4>
            <small class="text-white-50">Counter Clerk Panel</small>
        </div>
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    <h2 class="mb-0">Clerk Dashboard</h2>
                    <small class="text-muted">
                        {{ $agency->name }} - {{ optional($agency->city)->name ?? 'Unknown City' }}
                    </small>
                </div>
                <div class="dropdown">
                    <form method="POST" action="{{ route('profile.photo.update') }}" enctype="multipart/form-data" id="profilePhotoFormClerk" class="d-none">
                        @csrf
                        <input type="file" id="profilePhotoInputClerk" name="photo" accept="image/*" onchange="this.form.submit()">
                    </form>
                    <button class="profile-trigger dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="profile-avatar-wrap" onclick="event.preventDefault(); event.stopPropagation(); document.getElementById('profilePhotoInputClerk').click();">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <h3>{{ number_format((float) $stats['cash_register_balance']) }} XAF</h3>
            </div>
        </div>
        ...
        <div class="row g-4">
            <div class="col-lg-7">
                <div class="content-card">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <a href="{{ route('counter_clerk.reservations.create') }}" class="btn btn-primary w-100 p-3">
                                <i class="bi bi-plus-circle"></i> New Reservation
                            </a>
                        </div>
                        <div class="col-md-6">
                            <a href="{{ route('counter_clerk.reservations') }}" class="btn btn-outline-primary w-100 p-3">
                                <i class="bi bi-ticket-perforated"></i> View Reservations
                            </a>
                        </div>
                        <div class="col-md-6">
                            <a href="{{ route('counter_clerk.cash_register') }}" class="btn btn-success w-100 p-3">
                                <i class="bi bi-unlock"></i> Open Register
                            </a>
                        </div>
                        <div class="col-md-6">
                            <a href="{{ route('counter_clerk.cash_register') }}" class="btn btn-danger w-100 p-3">
            ...
            <div class="col-lg-5">
                <div class="content-card">
                    ...
                    <div class="mb-2">
                        ...
                        <strong>{{ optional($agency->company)->name ?? 'N/A' }}</strong>
                    </div>
                    <div class="mb-2">
                        <small class="text-muted d-block">City</small>
                        <strong>{{ optional($agency->city)->name ?? 'N/A' }}</strong>
                    </div>
                    <div class="mb-2">
                        <small class="text-muted d-block">Phone</small>
                        <strong>{{ $agency->phone ?? 'N/A' }}</strong>
                    </div>
                    <div>
                        <small class="text-muted d-block">Address</small>
                        <strong>{{ $agency->full_address ?? 'N/A' }}</strong>
                    </div>
                </div>
            </div>
        </div>
